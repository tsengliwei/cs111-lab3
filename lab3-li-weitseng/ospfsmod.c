...
// ospfs_set_nwrites_to_crash(cmd, n)
static int
ospfs_set_crash(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	switch (cmd) {
	case IOCTL_CRASH:
...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		*
		* EXERCISE: Your code here */ // checked
...
ospfs_unlink(struct inode *dirino, struct dentry *dentry)
{
	if (nwrites_to_crash < -1) {
		eprintk("Invalid nwrites_to_crash\n");
		return -EFAULT;
	}
	else if (nwrites_to_crash > 0) {
		nwrites_to_crash--;
		eprintk("nwrites_to_crash = %d\n", nwrites_to_crash);
	}
	else if (nwrites_to_crash == 0) {
		return 0;
	}
...
{
	/* EXERCISE: Your code here */ // checked
	uint32_t blockno, firstinode = ospfs_super->os_firstinob;
	// Loop from the first freemap block until before the first inode block
	for (blockno = OSPFS_FREEMAP_BLK; blockno < firstinode; blockno++) {
		void *vec = ospfs_block(blockno);
		int i;
		for (i = 0; i < OSPFS_BLKBITSIZE; i++) {
			if (bitvector_test(vec, i)) {
				bitvector_clear(vec, i); // Mark as used
				return (blockno - OSPFS_FREEMAP_BLK) * OSPFS_BLKBITSIZE + i; // return block number
			}
		}
	}
...
{
	/* EXERCISE: Your code here */ // checked
	if (blockno < ospfs_super->os_firstinob) // out of permitted bound (ignores checking inode block)
		return;
	uint32_t freemap_blockno = blockno / OSPFS_BLKBITSIZE + OSPFS_FREEMAP_BLK;
	if (freemap_blockno < ospfs_super->os_firstinob) // boundary checking
		bitvector_set(ospfs_block(freemap_blockno), blockno % OSPFS_BLKBITSIZE);
}
...
add_block(ospfs_inode_t *oi)
{
	...
	int32_t index = direct_index(n); // index for direct, indirect and indirect^2

	if (indirIndex == -1) {
		uint32_t blockno = allocate_block();
		if (!blockno)
			return -ENOSPC;
		memset(ospfs_block(blockno), 0, OSPFS_BLKSIZE); // zero out
		oi->oi_direct[index] = blockno;
	}
	else if (indir2Index == -1) {
		if (index == 0) { // allocate a new indirect block
			if (!(allocated[0] = allocate_block()))
				return -ENOSPC;
			memset(ospfs_block(allocated[0]), 0, OSPFS_BLKSIZE); // zero out
			oi->oi_indirect = allocated[0];
		}
		uint32_t *indirBlockPtr = ospfs_block(oi->oi_indirect);
		...
		memset(ospfs_block(allocated[1]), 0, OSPFS_BLKSIZE); // zero out
		indirBlockPtr[index] = allocated[1];
	}
	else if (indir2Index == 0) {
		if (indirIndex == 0 && index == 0) { // allocate a new indirect^2  block
			if (!(allocated[0] = allocate_block()))
				return -ENOSPC;
			memset(ospfs_block(allocated[0]), 0, OSPFS_BLKSIZE); // zero out
			oi->oi_indirect2 = allocated[0];
		}
		uint32_t *indir2BlockPtr = ospfs_block(oi->oi_indirect2);
		...
		if (index == 0) {
			...
			memset(ospfs_block(allocated[1]), 0, OSPFS_BLKSIZE); // zero out
			indir2BlockPtr[indirIndex] = allocated[1];
		}
		uint32_t *indirBlockPtr = ospfs_block(indir2BlockPtr[indirIndex]);

		uint32_t blockno = allocate_block();
		if (!blockno) {
			if (index == 0) {
				free_block(allocated[1]);
				indir2BlockPtr[indirIndex] = 0;
				if (indirIndex == 0) {
					free_block(allocated[0]);
					oi->oi_indirect2 = 0;
				}
...
remove_block(ospfs_inode_t *oi)
{
	...
	if (indirIndex == -1) {
		...
		uint32_t *indirBlockPtr = ospfs_block(oi->oi_indirect);
		free_block(indirBlockPtr[index]);
		if (index == 0) { // The indirect block is empty
			free_block(oi->oi_indirect);
			oi->oi_indirect = 0;
		}
		else
			indirBlockPtr[index] = 0;
	}
	else if (indir2Index == 0) {
		uint32_t *indir2BlockPtr = ospfs_block(oi->oi_indirect2);
		uint32_t *indirBlockPtr = ospfs_block(indir2BlockPtr[indirIndex]);
		...
		free_block(indirBlockPtr[index]);
		if (index == 0) {
			free_block(indir2BlockPtr[indirIndex]);
			if (indirIndex == 0) {
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	/* EXERCISE: Your code here */ // checked
	uint32_t fp = *f_pos + count; // file position after being written
	if (fp > oi->oi_size && change_size(oi, fp)) {
		retval = -EIO;
		goto done;
	}
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	for (; offsets < dir_oi->oi_size; offsets += OSPFS_DIRENTRY_SIZE) {
		ospfs_direntry_t* empty_entry = (ospfs_direntry_t*)ospfs_inode_data(dir_oi, offsets);
		if (empty_entry->od_ino == 0) {
			return empty_entry;
		}
	}
	...
	if (retval != 0)     //if fail to add block, return 
		return ERR_PTR(retval);
	else
		return (ospfs_direntry_t*)ospfs_inode_data(dir_oi, offsets);
}
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	if (find_direntry(dir_oi, dst_dentry->d_name.name, dst_dentry->d_name.len) != NULL)
		return -EEXIST; //check if a file already exists

	if (dst_dentry->d_name.len > OSPFS_MAXNAMELEN)
		return -ENAMETOOLONG; //check if name length too long

	ospfs_direntry_t* new_dir_entry = create_blank_direntry(dir_oi);

	if (IS_ERR(new_dir_entry))
		return PTR_ERR(new_dir_entry); // error checking
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	if (dentry->d_name.len>OSPFS_MAXNAMELEN)
		return -ENAMETOOLONG; //check if a file already exists


	if (find_direntry(dir_oi, dentry->d_name.name, dentry->d_name.len) != NULL)
		return -EEXIST; //check if name length too long


	ospfs_direntry_t* new_file_entry = create_blank_direntry(dir_oi);  //create new file entry
	if (IS_ERR(new_file_entry))
		return PTR_ERR(new_file_entry);

	//find inode and put it into the entry
	int current_inode_num = 2;
	ospfs_inode_t* current_inode;
	for (; current_inode_num < ospfs_super->os_ninodes; current_inode_num++) {
		current_inode = ospfs_inode(current_inode_num);
		if (current_inode->oi_nlink == 0) { //take the first empty inode
			break;
		}
	}
	if (current_inode_num >= ospfs_super->os_ninodes)
		return -ENOSPC; // error checking

	//Initialize the directory entry and inode
	new_file_entry->od_ino = current_inode_num;
	memcpy(new_file_entry->od_name, dentry->d_name.name, dentry->d_name.len);
	new_file_entry->od_name[dentry->d_name.len] = 0;
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	if (strncmp(oi->oi_symlink, "root?", 5) == 0){
		char *offset = strchr(oi->oi_symlink, ':');
		if (offset != NULL)
			*offset = '\0';
		else
			offset = strchr(oi->oi_symlink, '\0');

		real_offset = offset - oi->oi_symlink + 1;

		if (current->uid == 0)
			nd_set_link(nd, oi->oi_symlink + 5);
		else
			nd_set_link(nd, oi->oi_symlink + real_offset);
	}
	else
